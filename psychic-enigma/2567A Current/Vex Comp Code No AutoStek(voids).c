...
void sc()
{
	...
	(SensorValue[SAPotentiometer] = 0);
	(SensorValue[ClawPot] = 0);
}
void drive(int speed)
{
	motor[LDMotors] = speed; //move forward till swing arm is 45 degrees
	motor[RDMotors] = speed;
}

void driveDist(int speed, int count)
{
	Drive(speed);
...
{
	motor[LDMotors]= lSpeed
	motor[RDMotors]= rSpeed
}
void turndist (int lSpeed, int rSpeed, int count)
{
	motor[LDMotors]= lSpeed
	motor[RDMotors]= rSpeed
	untilEncoderCounts(count, LeftEncoder);
}
void swingArm(int speed,)
{
	motor[LSAMotor] = speed;
	motor[RSAMotor] = speed; //Extends Swing Arm
}
void swingArmTime(int speed, int Msec)
{
	swingArm(speed);
	wait1Msec(Msec);
}
void mg(int speed)
{
	motor[MGLiftMotorL] = speed;
	motor[MGLiftMotorR] = speed;
}
void mgTime(int speed, int Msec)
{
	mg(speed);
	wait1Msec(Msec);
}
