...
task main()
{
	while (true)
	{
		...
		{
			motor[LClaw] = 127;				//motor code for left claw motor opening
		}

		else if(vexRT[Btn5D] == 1)	//motor code for claw closing
		{
			motor[LClaw] = -127;			//motor code for Left claw motor closing
		}

		else												//stop for claw motors
		{
			motor[LClaw] = 0;					// end motor code for left claw motor
		}
